/**
 * 음성 온보딩 화면 — dot sphere + SpeechRecognition + StepSelector.
 */
...
export function createVoiceOnboarding({ onComplete, onFallback }) {
  ...
  transcriptEl.textContent = '듣고 있어요…';
  ...
  fallbackBtn.textContent = '텍스트로 선택할게요';
  ...
  function startRecognition() {
    recognizer = createRecognizer({
      onInterim(text) { transcriptEl.textContent = text || '듣고 있어요…'; },
      onFinal(text) {
  ...
  async function init() {
    ...
    try {
      ...
      // 마이크 권한 거부 → 텍스트 모드로 전환
      transcriptEl.textContent = '마이크 권한이 필요해요. 텍스트 모드로 전환합니다.';
      setTimeout(onFallback, 1800);
      return;
    }
